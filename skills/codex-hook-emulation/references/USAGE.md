...
```bash
python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" preflight "git reset --hard HEAD~1"
```
...
```bash
python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" session-start --cwd "$PWD" --json
python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" preflight --json "git push --force origin main"
```
