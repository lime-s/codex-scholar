...
1. **SessionStart surrogate**
   - Use `"$CODEX_HOME/scripts/codex_hook_emulation.py" session-start --cwd "$PWD"` at the first substantive repo turn when the helper exists.
2. **PreToolUse surrogate**
   - Before dangerous or irreversible operations, run:
     - `python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" preflight "<command>"`
...
1. Run `python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" session-end --cwd "$PWD"` when the helper exists
...
- Prefer `python3 "$CODEX_HOME/scripts/codex_hook_emulation.py" preflight "<command>"` before high-risk shell actions
